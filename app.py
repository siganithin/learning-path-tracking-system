...
if section == "Students":
    ...
    if selected_plan:
        if st.button("Delete Study Plan"):
            try:
                ...
                st.error(f"Error deleting study plan: {e}")
    ...
    with st.form("add_plan_form"):
        ...
        if submitted and title and student_choice and subject_choice:
            try:
                ...
                st.error(f"Error creating study plan: {e}")

    st.header("List of Study Plans")
    plans = StudyPlanService.list_studyplans()
    # Calculate and show completion progress per plan
    table_data = []
    for p in plans:
        tasks = TaskService.list_tasks(p["plan_id"])
        total = len(tasks)
    ...
    st.table(table_data if table_data else [])
    ...
    with st.form("add_task_form"):
        ...
        if submitted:
            if not plan_choice or not description or not deadline:
                st.error("Please fill all fields.")
            elif not validate_date(deadline):
                st.error("Enter deadline in YYYY-MM-DD format.")
            else:
                try:
    ...
    try:
        ...
        reports = ReportService.view_report(None)
    # Deduplicate by plan ID for display
    unique_reports = {}
    for r in reports:
        pid = r["plan_id"]
        if pid not in unique_reports:
            unique_reports[pid] = r
    st.table([{ "Report ID": rep.get("report_id", "N/A"), "Plan ID": rep["plan_id"], "Summary": rep["summary"] }
